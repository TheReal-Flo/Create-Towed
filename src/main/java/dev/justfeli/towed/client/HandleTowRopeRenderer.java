...
public final class HandleTowRopeRenderer implements BlockEntityRenderer<HandleBlockEntity> {
    public HandleTowRopeRenderer(final BlockEntityRendererProvider.Context context) {
    }

    @Override
    public void render(final HandleBlockEntity blockEntity, final float partialTick, final PoseStack poseStack, final MultiBufferSource buffer, final int packedLight, final int packedOverlay) {
        final ClientTowRopeManager manager = ClientTowRopeManager.getOrCreate(blockEntity.getLevel());
        if (manager == null) {
            return;
        }

        final Iterable<TowedClientRopeStrand> ropes = manager.getForHandle(blockEntity.getBlockPos());
        final BlockPos ownerPos = blockEntity.getBlockPos();
        ...
        final Pose3dc containingPose = subLevel instanceof ClientSubLevel clientSubLevel ? clientSubLevel.renderPose() : null;

        for (final TowedClientRopeStrand rope : ropes) {
            ...
            for (int i = 1; i < renderPoints.size(); i++) {
                ...
                final Vector3d renderPos = new Vector3d(point0.position());
                ...
                poseStack.pushPose();
                poseStack.translate(renderPos.x - ownerPos.getX(), renderPos.y - ownerPos.getY(), renderPos.z - ownerPos.getZ());
                poseStack.mulPose(orientation);
                poseStack.translate(-0.5, -0.5, -0.5);

                final int worldLight = LevelRenderer.getLightColor(blockEntity.getLevel(), BlockPos.containing(globalRenderPos.x, globalRenderPos.y, globalRenderPos.z));
                if (i > 1) {
    ...
    @Override
    public boolean shouldRenderOffScreen(final HandleBlockEntity blockEntity) {
        return true;
    }
